		...
		if (i % 2 != SUCCESS) 
		{
			if (argv[i][0] == '-') 
			{
				if (strcmp(argv[i], REQ_ARRIVAL_RATE) == SUCCESS) //r
				{
					if (argv[i + 1] == NULL) 
					{
						error_logger("r is NULL, Enter Request Arrival Rate > Zero(0)");
						exit(FAILURE);
					}
					r = atof(argv[i + 1]);
					if (r == SUCCESS) 
					{
						...
						{
							error_logger("Request Arrival Rate cannot be Zero(0)");
							exit(FAILURE);
						} 
						else 
						{
				...
				else if (strcmp(argv[i], REQ_SERVING_TIME) == SUCCESS) //t
				{
					if (argv[i + 1] == NULL) 
					{
						error_logger("t is NULL, Enter Request Serving Time > Zero(0)");
						exit(FAILURE);
					}
					t = atof(argv[i + 1]);
					if (t == SUCCESS) 
					{
						if (strcmp(INVALID_VALUE, argv[i + 1]) == SUCCESS) 
						{
							error_logger("Request Serving Time cannot be Zero(0)");
							exit(FAILURE);
						} 
						else 
						{
				...
				else if (strcmp(argv[i], RESOURCE_TOKEN_RATE) == SUCCESS) //x
				{
					if (argv[i + 1] == NULL) 
					{
						error_logger("x is NULL, Enter Resource Token Rate > Zero(0)");
						exit(FAILURE);
					}
					x = atof(argv[i + 1]);
					if (x == SUCCESS) 
					{
						if (strcmp(INVALID_VALUE, argv[i + 1]) == SUCCESS) 
						{
							error_logger("Resource Token Rate cannot be Zero(0)");
							exit(FAILURE);
						} 
						else 
						{
				...
				else if (strcmp(argv[i], FINITE_BUFFER) == SUCCESS)  //L
				{
					if (argv[i + 1] == NULL) 
					{
						error_logger("L is NULL, Enter Token Queue Size > Zero(0)");
						exit(FAILURE);
					}
					L = atoi(argv[i + 1]);
					if (L == SUCCESS) 
					...
					if (L > MAX_FINITE_BUFFER_LENGTH) 
					{
						error_logger("L cannot be greater than MAX_FINITE_BUFFER_LENGTH = %d", 
						MAX_FINITE_BUFFER_LENGTH);
						exit(FAILURE);
					}
				} 
				...
				else if (strcmp(argv[i], TOKENS_TO_SERVE_REQ) == SUCCESS) //R
				{
					if (argv[i + 1] == NULL) 
					{
						error_logger("R is NULL, Enter Token to serve request > Zero(0)");
						exit(FAILURE);
					}
					R = atoi(argv[i + 1]);
					if (R == SUCCESS) 
					{
						if (strcmp(INVALID_VALUE, argv[i + 1]) == SUCCESS) 
						{
							error_logger("Token to serve request cannot be ZERO(0)");
							exit(FAILURE);
						} 
						else 
						{
					...
					if (R > MAX_FINITE_BUFFER_LENGTH) 
					{
						error_logger("R cannot be greater than MAX_FINITE_BUFFER_LENGTH = %d", 
						MAX_FINITE_BUFFER_LENGTH);
						exit(FAILURE);
					}
				} 
				...
				{
					error_logger("Command Line Argument is Invalid\n"
			...
			{
				error_logger("Invalid Command line arguments, Use valid argument[-r r, -R R, -L L, -x x, -t t]");
				exit(FAILURE);
			}
		}
	}
